...
std::vector<CombNormal> find_optimal_points(DataFrame df, std::vector<std::string> input_etf_list) {
	...
	for (CombNormal datapoint : pointlist) {
		std::cout << "Weight 1: " << datapoint.ETF_weights[0].first << ", " << datapoint.ETF_weights[0].second << std::endl;
		std::cout << "Weight 2: " << datapoint.ETF_weights[1].first << ", " << datapoint.ETF_weights[1].second << std::endl;
		std::cout << datapoint.mean << ", " << datapoint.stdev << std::endl << std::endl;
	}

	std::cout << normallist[0].stdev << ", " << normallist[1].stdev << std::endl;
